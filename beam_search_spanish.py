...
with open(
    r"C:\Users\aslin\NullClass_Task-1\Task 1\eng_vectorization_config.json"
) as json_file:
...
with open(
    r"C:\Users\aslin\NullClass_Task-1\Task 1\spa_vectorization_config.json"
) as json_file:
...
with open(r"C:\Users\aslin\NullClass_Task-1\Task 1\eng_vocab.json") as json_file:
    eng_vocab = json.load(json_file)
    eng_vectorization.set_vocabulary(eng_vocab)

# Load and set the Spanish vocabulary
with open(r"C:\Users\aslin\NullClass_Task-1\Task 1\spa_vocab.json") as json_file:
    spa_vocab = json.load(json_file)
    spa_vectorization.set_vocabulary(spa_vocab)

# Load the Spanish model
transformer = load_model(r"C:\Users\aslin\NullClass_Task-1\Task 1\transformer_model")
...
# Load French model
model = load_model(r"C:\Users\aslin\NullClass_Task-1\Task 1\english_to_french_model")

# Load Tokenizer
with open(r"C:\Users\aslin\NullClass_Task-1\Task 1\english_tokenizer.json") as f:
    data = json.load(f)
    english_tokenizer = tokenizer_from_json(data)

with open(r"C:\Users\aslin\NullClass_Task-1\Task 1\french_tokenizer.json") as f:
    data = json.load(f)
    french_tokenizer = tokenizer_from_json(data)

# Load max length
with open(r"C:\Users\aslin\NullClass_Task-1\Task 1\sequence_length.json") as f:
...
# Heading for output
output_heading = tk.Label(
    output_frame, text="Translation: ", font=(font_style, font_size, "bold")
)
output_heading.pack()

# Text output for translations
translation_output = tk.Text(
    output_frame, height=10, width=50, font=(font_style, font_size)
)
